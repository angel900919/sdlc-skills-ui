# Root CLAUDE.md seed template

Referenced from SKILL.md Phase 8. Copy the block below to the project root as `CLAUDE.md` (or `CLAUDE.md.suggested` if one already exists). Resolve the two placeholders first:

- `{{PROJECT_NAME}}` — prefer `package.json#name` (brownfield ts/js), then `pyproject.toml [project] name` / `go.mod` module / `Cargo.toml [package] name`, else the project-root directory basename.
- `{{ONE_LINE_DESCRIPTION}}` — prefer the idea one-liner from `.ai/intake.md` / `.human/intake/idea.md`; else `package.json#description`; else leave the placeholder and prepend `<!-- TODO: fill this in -->`.

Trim sections that don't apply to the project's tier (see `.ai/anchor.md`). The "Deeper references" table points only at files that exist today; chain files are added as later skills are built — leave the noted entries as forward references.
