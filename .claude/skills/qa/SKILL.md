...
3. **Delegate execution; degrade gracefully.** Hand the mechanical runs to built-ins — **`/verify`** (acceptance behavior + regression), **`/run`** (bring the app up for the human script), **`/security-review`** (production), and the `fitness/` run. If a built-in is absent (older CC / the Windows checkout), **emit the manual command for the human and WARN** — never silently skip a gate. Version floors + fallbacks: [references/checks.md](references/checks.md).
