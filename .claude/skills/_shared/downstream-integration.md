...
/quick-spec   → .ai/specs/<feature>/prd.md + design.md + plan.md in ONE sitting  (N×, prototype tier ONLY)  [BUILT]  ← express lane; runs the prototype columns of /prd → /design → /plan as one conversation; schema-identical artifacts (no new artifact kind, no ai-schema addition); design's .human mirror still written; effective-tier gate refuses mvp+/uplift (NEEDS-FULL-CHAIN → /prd — non-overridable in-skill, the override lives in /prd); partial spec trio → RESUME-FULL-CHAIN; success READY-FOR-ISSUES → /to-issues
