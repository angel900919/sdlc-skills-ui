...
# (b) Derived .human mirrors — ask. The .human tree is a projection of .ai/,
# regenerated by the producing skill; a hand-edit here is overwritten and drifts.
# .human/intake/ is the exception: human-authored input, not a mirror, so it
# passes before the .human pattern can match. Ask not deny — a human may have a
# deliberate reason and the hook stdin carries no caller identity.
case "$path" in
    */.human/intake/*) exit 0 ;;
esac
case "$path" in
    */.human/*)
        decide ask ".human/ files are derived projections of .ai/ (regenerated by the producing skill). Edit the corresponding .ai/ source and re-run its skill instead - hand-edits here are overwritten and cause documentation drift." ;;
esac

# (c) Gate self-protection — ask. The agent being graded must not silently
...
# (d) Frozen canonical specs — ask, content-aware. Only the frontmatter is
