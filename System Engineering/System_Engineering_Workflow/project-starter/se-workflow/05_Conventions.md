...
- **Notation:** PlantUML (`.puml`) — text‑as‑diagram, diffs cleanly in git, renders via `plantuml *.puml` or the VS Code PlantUML extension. Escalate to a true MBSE tool (Cameo, Capella, Sparx) when a live model/digital‑thread is required — see Stage 03 and the SysML‑v2 note there.
