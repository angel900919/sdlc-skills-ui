...
## Rules
- **Conform to Conventions for everything shared.** IDs (`ICD-<nn>`, `DEC-<nn>` placeholders, `REQ-*`), gates, baselines, T/I/A/D, severity `S1`–`S4`, status strings, PlantUML conventions, and standard citations all come from [`../../05_Conventions.md`](../../05_Conventions.md). Cross-reference other phases; never restate their conventions here.
...
Blank versions live in [`../../templates/`](../../templates/). Compact skeletons:
...
## Cross-cutting hooks
This phase feeds and consumes these threads (link each in [`../../cross-cutting/`](../../cross-cutting/)):
- **Security** — the Physical/Security view's **trust boundaries** are the inputs to threat modeling; every boundary-crossing ICD row should map to a `THR-*` and an auth mechanism. → [`../../cross-cutting/Security_Engineering.md`](../../cross-cutting/Security_Engineering.md).
- **Safety / RAMS** — safety-relevant interfaces and blocks feed the hazard log; allocate reliability/availability budgets to blocks here. → [`../../cross-cutting/Safety_RAMS_Engineering.md`](../../cross-cutting/Safety_RAMS_Engineering.md).
- **Risk & Opportunity** — architectural dependencies and single points of failure raise `RSK-*`; High/Critical risks block PDR. → [`../../cross-cutting/Risk_and_Opportunity_Management.md`](../../cross-cutting/Risk_and_Opportunity_Management.md).
- **Configuration Mgmt** — the architecture + ICD set + allocation matrix *are* the **allocated baseline** established at PDR (frozen for change control in Phase 09). → [`../../cross-cutting/Configuration_Management.md`](../../cross-cutting/Configuration_Management.md).
- **Measurement (MOE/MOP/TPM)** — concerns trace to MOEs; performance budgets in the ICD trace to MOPs/TPMs tracked through Phases 06–10. → [`../../cross-cutting/Measurement_MOE_MOP_TPM.md`](../../cross-cutting/Measurement_MOE_MOP_TPM.md).
...
## References
- [`../../05_Conventions.md`](../../05_Conventions.md) — IDs, gates (PDR/CDR), baselines, T/I/A/D, severity, status strings, diagrams, standard citations (the contract).
- [`../../01_Workflow_Overview.md`](../../01_Workflow_Overview.md) — the 12-stage spine, V-model, 15288 process map, 8 threads.
